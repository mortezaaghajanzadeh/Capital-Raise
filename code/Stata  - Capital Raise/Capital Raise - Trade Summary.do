...
graph export IndImb_NoRevaluation.eps,replace
  ...
  twoway connected inslimbalance  eperiod   if (RaiseType2 == "NoRevaluation") , xline(0) yline(0) xlab(`l'(10)`u')  msymbol(O D) title("Institutional imbalances After Capital that not from revaluation",size(medium))   xtitle("Period") color(navy)note("This figure graphs the buy-sell imbalances.""It is defined as the net buying ratio of stock k on date t by a particular type to the amounts bought ""and sold by that type.")   ytitle("Buy-Sell Imbalances")   ylabe(-0.2(0.1)0.2,angle(0)labsize(vsmall)) sort(eperiod)
...
twoway connected lnVolume  eperiod   if (RaiseType2 == "Revaluation") , xline(0) xlab(`l'(10)`u')  msymbol(O D) title("Volume  After Capital Raise from Revaluation",size(medium))   xtitle("Period")  color(navy)  ytitle("ln(Volume) ") ylabe(,angle(0) labsize(vsmall)) sort(eperiod)

graph export volume_Revaluation.png,replace
graph export volume_Revaluation.eps,replace

twoway connected relvolume  eperiod   if (RaiseType2 == "Revaluation") , xline(0)  xlab(`l'(10)`u')  msymbol(O D) title("Relative Volume  After Capital Raise from Revaluation",size(medium))   xtitle("Period")  color(navy)  ytitle(" Relative Volume") ylabe(,angle(0) labsize(vsmall)) yline(1) sort(eperiod)

graph export relvolume_Revaluation.png,replace
graph export relvolume_Revaluation.eps,replace


twoway connected lnAmihud  eperiod   if (RaiseType2 == "Revaluation") , xline(0)  xlab(`l'(10)`u')  msymbol(O D) title("Amihud measurment  After Capital Raise from Revaluation",size(medium))   xtitle("Period")  color(navy)  ytitle("ln(Amihud) ")ylabe(,angle(0) labsize(vsmall)) sort(eperiod)
...
twoway connected lnVolume  eperiod   if (RaiseType2 == "NoRevaluation") , xline(0) xlab(`l'(10)`u')  msymbol(O D) title("Volume  After Capital Raise that it's not from Revaluation",size(medium))   xtitle("Period")  color(navy)  ytitle("ln(Volume) ") ylabe(,angle(0) labsize(vsmall))  sort(eperiod)

graph export volume_NoRevaluation.png,replace
graph export volume_NoRevaluation.eps,replace

twoway connected relvolume  eperiod   if (RaiseType2 == "NoRevaluation") , xline(0)  xlab(`l'(10)`u')  msymbol(O D) title("Relative Volume  After Capital Raise  that it's not from Revaluation",size(medium))   xtitle("Period")  color(navy)  ytitle(" Relative Volume") ylabe(,angle(0) labsize(vsmall)) yline(1) sort(eperiod)

graph export relvolume_NoRevaluation.png,replace
graph export relvolume_NoRevaluation.eps,replace


twoway connected lnAmihud  eperiod   if (RaiseType2 == "NoRevaluation") , xline(0)  xlab(`l'(10)`u')  msymbol(O D) title("Amihud measurment  After Capital Raise that it's not from Revaluation",size(medium))   xtitle("Period")  color(navy)  ytitle("ln(Amihud) ")ylabe(,angle(0) labsize(vsmall)) sort(eperiod)
