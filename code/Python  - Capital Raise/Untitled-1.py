...
# %%
a = df[['JustRO', 'JustSaving','Revaluation', 'Hybrid']].sum().to_frame().T.rename(index = {0:"Event"})
b = (df[['JustRO', 'JustSaving','Revaluation', 'Hybrid']].describe().T.iloc[:,1]*100).round(2).to_frame().T.rename(index = {'mean':"Percent"})
a.append(b)
# %%
df[df.name == 'خمهر']
...
# %%
a = df.groupby('Q').JustSaving.sum().to_frame().rename(columns = {'JustSaving':'Reserves'}).T
a = a.append(df.groupby('Q').JustRO.sum().to_frame().rename(columns = {'JustRO':'Cash'}).T)
a = a.append(df.groupby('Q').JustPremium.sum().to_frame().rename(columns = {'JustPremium':'Premium'}).T)
a = a.append(df.groupby('Q').Hybrid.sum().to_frame().rename(columns = {'Hybrid':'Hybrid'}).T)
a = a.append(df.groupby('Q').Revaluation.sum().to_frame().rename(columns = {'Revaluation':'Revaluation'}).T)
...
# %%
a = df.groupby('yearQ').JustSaving.sum().to_frame().rename(columns = {'JustSaving':'Saving'}).T
a = a.append(df.groupby('yearQ').JustRO.sum().to_frame().rename(columns = {'JustRO':'Cash'}).T)
a = a.append(df.groupby('yearQ').JustPremium.sum().to_frame().rename(columns = {'JustPremium':'Premium'}).T)
a = a.append(df.groupby('yearQ').Hybrid.sum().to_frame().rename(columns = {'Hybrid':'Hybrid'}).T)
a = a.append(df.groupby('yearQ').Revaluation.sum().to_frame().rename(columns = {'Revaluation':'Revaluation'}).T)
...
a[['Cash','Saving', 'Hybrid', 'Revaluation','Premium']].plot.bar(stacked=True, figsize=(20,10))
plt.title("Number of Capital Raise")
txt= "Note: Number of Capital Raise from each source "
plt.xlabel('Year Quarter ')
plt.figtext(0.5,-0.005, txt, horizontalalignment='center',
...
# %%
yearCapmedian = df.groupby('year').CapRaised.median().to_frame().rename(columns = {'CapRaised':'MedianCapRaise'})
yearCapmedian.plot.bar(stacked=False, figsize=(10,5))
yearCapmedian = yearCapmedian.reset_index()


# %%
df.groupby('year').get_group(1397).sort_values(by = ['CapRaised'],ascending = False)


# %%
yearsum = df.groupby('year').CapRaised.sum().to_frame().rename(columns = {'CapRaised':'Sum'})
...
# %%
len(df[(df.year == 1383)&(df.Percent == 100)].sort_values(by = ['Percent']))


# %%
yearmean = df.groupby('year').Percent.median().to_frame().rename(columns = {'Percent':'MedianPercent'})
...
# %%
vdata = yearnumber.merge(yearmean).merge(yearsum).merge(yearCapmedian).merge(yearHybridnumber).merge(yearPremiumnumber).merge(yearSavingnumber).merge(yearROnumber).merge(yearRevaluationnumber)
...
# %%
df.loc[df.Revaluation == 1][['name','JustRO', 'JustSaving', 'JustPremium',
       'Hybrid', 'Revaluation',]]


# %%
a = df[['JustRO', 'JustSaving', 'JustPremium','Revaluation', 'Hybrid']].sum().to_frame().T.rename(index = {0:"Event"})
b = (df[['JustRO', 'JustSaving', 'JustPremium','Revaluation', 'Hybrid']].describe().T.iloc[:,1]*100).round(2).to_frame().T.rename(index = {'mean':"Percent"})
a.append(b)


# %%
df[df.JustRO == 1].JustRO.sum()
